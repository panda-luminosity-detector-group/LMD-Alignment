...
class histogram(object):
    ...
    def histBinaryPairFileSingle_dx_dy(self, path, overlapID, cut, axes):
        ...
        axes.hist2d(dHit[:,0], dHit[:,1], bins=150, norm=LogNorm(), range=[[-0.12, 0.12], [-0.12, 0.12]])
        ...
        axes.set_title('mis: dx: {:1.2f}µm, dy: {:1.2f}µm'.format( toSen1[0][3], toSen1[1][3]))
    ...
    def histBinaryPairFileSingle_x_dx(self, path, overlapID, cut, axes):
        ...
        axes.hist2d(hit1T[:,0], dHit[:,0]*dHit[:,0]*1e4 + dHit[:,1]*dHit[:,1]*1e4, bins=250, norm=LogNorm())    # , range=[[-0.12, 0.12], [-0.12, 0.12]]
    ...
    def plotAllhitsonCirlce(self):
        ...
        hist = axes.hist2d(hitsAllOne[:,0], hitsAllOne[:,1], bins=500, norm=LogNorm(), cmap=plt.get_cmap('viridis'))
        fig.tight_layout(rect=[0, 0.03, 1, 0.95])
        plt.colorbar(hist[3])
        plt.show()
