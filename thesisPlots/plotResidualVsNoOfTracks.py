...
def plotNIter():
    ...
    if True:
        jsonFiles = Path('output/residualVsIterations').glob('[0-9]*.json')

        lines = []
        for file in jsonFiles:
            m = re.search(r'residualVsIterations/(\d+)-VsResiduals.js', str(file))
            if m:
                nTrk = float(m.group(1))
            else:
                continue
    ...
    # else:
    #     with open(f'output/residualVsIteraions/nTrksVsResiduals-Hand.json', 'r') as f:
    #         data = json.load(f)
    #     lines = []
    #     for nTrk in data:
    #         values = np.array(data[nTrk]).reshape((40, 3))
