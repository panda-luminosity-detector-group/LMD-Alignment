...
def plot(valuesUncorrected, valuesCorrected, outFileName):
    ...
    titlesCorr = [
        "Luminosity Fit Error, Misaligned Sensors",
        "Luminosity Fit Error, Misaligned Sensors",
    ...
    for i in range(len(sizes)):
        ...
        for mom in momenta:
            ...
            ax.errorbar(
                thseValsTwo[:, 1] * 100 - 2.5 * iMa + 2.5,
                thseValsTwo[:, 2],
                yerr=thseValsTwo[:, 3],
                fmt=markers[iMa + 1],
                ecolor="black",
                color=colors[colorI + 1],
                capsize=2,
                elinewidth=0.6,
                label=f"{mom} GeV \nAligned",
                ls="dashed",
        ...
        ax.legend(
            handles, labels, loc="lower left", numpoints=1, bbox_to_anchor=(1, -0.1)
        )
...
if __name__ == "__main__":
    ...
    for mom in momenta:
        for mis in misaligns:
            ...
            valuesUncorrected.append(
                [
                    float(mom),
                    float(mis),
                    float(fValF["relative_deviation_in_percent"][0]),
                    float(fValF["relative_deviation_error_in_percent"][0]),
                ]
            )
            valuesCorrected.append(
                [
                    float(mom),
                    float(mis),
                    float(fValT["relative_deviation_in_percent"][0]),
                    float(fValT["relative_deviation_error_in_percent"][0]),
                ]
            )
