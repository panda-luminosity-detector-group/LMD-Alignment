...
def startLogToFile(functionName=None):
    ...
    if functionName is None:
        ...
        runSimLog = (
            f"runLogs/{datetime.date.today()}/run{runNumber}-main-{functionName}.log"
        )
        runSimLogErr = f"runLogs/{datetime.date.today()}/run{runNumber}-main-{functionName}-stderr.log"

    # redirect stdout/stderr to log files
    print(
        f"+++ starting new run and forking to background! this script will write all output to {runSimLog}\n"
    )
...
def runAligners(runConfig, threadID=None):
    ...
    sensorAlignerOverlapsResultName = runConfig.pathAlMatrixPath() / Path(
        f"alMat-sensorOverlaps-seed{config.seedID}-{runConfig.misalignFactor}.json"
    )
    sensorAlignerResultName = runConfig.pathAlMatrixPath() / Path(
        f"alMat-sensorAlignment-seed{config.seedID}-{runConfig.misalignFactor}.json"
    )
    moduleAlignerResultName = runConfig.pathAlMatrixPath() / Path(
        f"alMat-moduleAlignment-seed{config.seedID}-{runConfig.misalignFactor}.json"
    )
    IPalignerResultName = runConfig.pathAlMatrixPath() / Path(
        f"alMat-IPalignment-seed{config.seedID}-{runConfig.misalignFactor}.json"
    )
    mergedAlignerResultName = runConfig.pathAlMatrixPath() / Path(
        f"alMat-merged-seed{config.seedID}-{runConfig.misalignFactor}.json"
    )
    combi0Name = runConfig.pathAlMatrixPath() / Path(
        f"alMat-combiSen-seed{config.seedID}-{runConfig.misalignFactor}.json"
    )
    combi1Name = runConfig.pathAlMatrixPath() / Path(
        f"alMat-combiSenMod-seed{config.seedID}-{runConfig.misalignFactor}.json"
    )
    combi2Name = runConfig.pathAlMatrixPath() / Path(
        f"alMat-combiSenIP-seed{config.seedID}-{runConfig.misalignFactor}.json"
    )
    combi3Name = runConfig.pathAlMatrixPath() / Path(
        f"alMat-combiSenModIP-seed{config.seedID}-{runConfig.misalignFactor}.json"
    )
    ...
    thislogger = LMDrunLogger(
        f"./runLogs/{datetime.date.today()}/run{runNumber}-worker-Alignment-{runConfig.misalignType}-{runConfig.misalignFactor}-th{threadID}.txt"
    )
    ...
    sensorAligner.loadExternalMatrices(
        runConfig.sensorAlignExternalMatrixPath
    )  # TODO: do implicitly, aligners always start with a run config
    ...
    moduleAligner.readAnchorPoints(
        runConfig.moduleAlignAnchorPointFile
    )  # TODO: do implicitly, aligners always start with a run config
    moduleAligner.readAverageMisalignments(
        runConfig.moduleAlignAvgMisalignFile
    )  # TODO: do implicitly, aligners always start with a run config
...
def runCombi(runConfig, threadID=None):
    ...
    if runConfig.alignmentCorrection or (
        runConfig.misalignType != "combi" and runConfig.misalignType != "aligned"
    ):
    ...
    # the follwong MUST be unique across all simulations, so set the seedID too!
    sensorAlignerOverlapsResultName = runConfig.pathAlMatrixPath() / Path(
        f"alMat-sensorOverlaps-seed{runConfig.seedID}-{runConfig.misalignFactor}.json"
    )
    sensorAlignerResultName = runConfig.pathAlMatrixPath() / Path(
        f"alMat-sensorAlignment-seed{runConfig.seedID}-{runConfig.misalignFactor}.json"
    )
    moduleAlignerResultName = runConfig.pathAlMatrixPath() / Path(
        f"alMat-moduleAlignment-seed{runConfig.seedID}-{runConfig.misalignFactor}.json"
    )
    IPalignerResultName = runConfig.pathAlMatrixPath() / Path(
        f"alMat-IPalignment-seed{runConfig.seedID}-{runConfig.misalignFactor}.json"
    )
    combi0Name = runConfig.pathAlMatrixPath() / Path(
        f"alMat-combiSen-seed{runConfig.seedID}-{runConfig.misalignFactor}.json"
    )
    combi1Name = runConfig.pathAlMatrixPath() / Path(
        f"alMat-combiSenMod-seed{runConfig.seedID}-{runConfig.misalignFactor}.json"
    )
    combi3Name = runConfig.pathAlMatrixPath() / Path(
        f"alMat-combiSenModIP-seed{runConfig.seedID}-{runConfig.misalignFactor}.json"
    )

    # create logger
    thislogger = LMDrunLogger(
        f"./runLogs/{datetime.date.today()}/run{runNumber}-worker-combi-{runConfig.misalignType}-{runConfig.misalignFactor}-th{threadID}.txt"
    )
    ...
    moduleAlignDataPath = (
        runConfig.pathTrksQA()
    )  # TODO: there is an error here! This doesn't go into the proper subfolders! (1-100_uncut/no_alignment_correction)
    moduleAlignTrackFile = moduleAlignDataPath / Path("processedTracks.json")
...
def runExtractLumi(runConfig, threadID=None):
    ...
    # create logger
    thislogger = LMDrunLogger(
        f"./runLogs/{datetime.date.today()}/run{runNumber}-worker-ExtractLumi-{runConfig.misalignType}-{runConfig.misalignFactor}-th{threadID}.txt"
    )
    thislogger.log(runConfig.dump())
...
def runLumifit(runConfig, threadID=None):
    ...
    # create logger
    thislogger = LMDrunLogger(
        f"./runLogs/{datetime.date.today()}/run{runNumber}-worker-LumiFit-{runConfig.misalignType}-{runConfig.misalignFactor}-th{threadID}.txt"
    )
    thislogger.log(runConfig.dump())
...
def runSimRecoLumi(runConfig, threadID=None):
    ...
    # create logger
    thislogger = LMDrunLogger(
        f"./runLogs/{datetime.date.today()}/run{runNumber}-worker-SimReco-{runConfig.misalignType}-{runConfig.misalignFactor}-th{threadID}.txt"
    )
    thislogger.log(runConfig.dump())
...
def halfRun(runConfig, threadID=None):
    ...
    # create logger
    thislogger = LMDrunLogger(
        f"./runLogs/{datetime.date.today()}/run{runNumber}-worker-FullRun-{runConfig.misalignType}-{runConfig.misalignFactor}-th{threadID}.txt"
    )
...
def showLumiFitResults(runConfigPath, threadID=None, saveGraph=False):
    ...
    if saveGraph:
        ...
        fileName = Path(
            f"output/LumiResults/{configs[0].momentum}/{configs[0].misalignType}/LumiFits-{corrStr}"
        )
        ...
        fileName2 = Path(
            f"output/LumiResults/All/multi-alMat-combiSenMod-{configs[0].misalignFactor}.json"
        )
        ...
        if (configs[0].misalignType == "combi") and (configs[0].seedID is not None):
            ...
            fileName2 = Path(
                f"output/LumiResults/All/{configs[0].misalignType}-{corrStr}"
            )
...
def histogramRunConfig(runConfig, threadId=0):
    ...
    targetDir = Path(
        f"output/temp/alMats/{runConfig.misalignType}-{runConfig.momentum}-{runConfig.misalignFactor}/"
    )
    ...
    if not importantFile.exists():
        ...
        subprocess.run(
            [
                ...
                Path(
                    targetDir
                    / Path(f"alMat-combiSenModIP-{runConfig.misalignFactor}.json")
                ),
                importantFile,
            ]
        )
    ...
    comparator.loadDesignMisalignments(
        "/media/DataEnc2TBRaid1/Arbeit/Root/PandaRoot-New/macro/detectors/lmd/geo/misMatrices/misMat-aligned-1.00.json"
    )
    comparator.loadAlignerMatrices(targetDir / Path(f"alMat-merged.json"))
    boxResult = comparator.saveHistogram(
        f"output/comparison/{runConfig.momentum}/misalign-{runConfig.misalignType}/box-{runConfig.misalignFactor}-icp.pdf"
    )
    ...
    comparator.loadDesignMisalignments(runConfig.misMatFile)
    comparator.loadSensorAlignerOverlapMatrices(
        targetDir / Path(f"alMat-sensorOverlaps-{runConfig.misalignFactor}.json")
    )
    overlapResult = comparator.saveHistogram(
        f"output/comparison/{runConfig.momentum}/misalign-{runConfig.misalignType}/sensor-overlaps-{runConfig.misalignFactor}-icp"
    )
    ...
    comparator.loadDesignMisalignments(runConfig.misMatFile)
    comparator.loadAlignerMatrices(targetDir / Path(f"alMat-merged.json"))
    moduleResult = comparator.saveHistogram(
        f"output/comparison/{runConfig.momentum}/misalign-{runConfig.misalignType}/modules-{runConfig.misalignFactor}"
    )
    ...
    comparator.loadDesignMisalignments(runConfig.misMatFile)
    comparator.loadAlignerMatrices(targetDir / Path(f"alMat-merged.json"))
    sensorResult = comparator.saveHistogram(
        f"output/comparison/{runConfig.momentum}/misalign-{runConfig.misalignType}/sensors-{runConfig.misalignFactor}-misalignments"
    )
    ...
    values[runConfig.momentum][runConfig.misalignFactor] = {
        ...
        "modules": (
            np.array(moduleResultMean).tolist(),
            np.array(moduleResultSigma).tolist(),
        ),
        "overlaps": (
            np.array(overlapResultMean).tolist(),
            np.array(overlapResultSigma).tolist(),
        ),
        "sensors": (
            np.array(sensorResultMean).tolist(),
            np.array(sensorResultSigma).tolist(),
        ),
    }
            ...
            for fac in misFactors[misType]:
                for corr in correctedOptions:
                    ...
                    if misType == "box100":
                        ...
                        config.alMatFile = str(
                            config.pathAlMatrixPath()
                            / Path(f"alMat-IPalignment-{fac}.json")
                        )
                    ...
                    if misType == "combi":
                        ...
                        config.alMatFile = str(
                            config.pathAlMatrixPath()
                            / Path(f"alMat-combiSenMod-{fac}.json")
                        )  # this is a placeholder, the actual values are in the special subfolder after makeCombiRunConfigs

                    if misType == "modules":
                        config.misMatFile = f"{vmcworkdir}/macro/detectors/lmd/geo/misMatrices/misMat-modules-{fac}.json"
                        config.alMatFile = str(
                            config.pathAlMatrixPath()
                            / Path(f"alMat-moduleAlignment-{fac}.json")
                        )
                    ...
                    if misType == "sensors":
                        ...
                        config.alMatFile = str(
                            config.pathAlMatrixPath()
                            / Path(f"alMat-sensorAlignment-{fac}.json")
                        )
                    ...
                    if misType == "aligned" or misType == "identity":
                        ...
                        config.sensorAlignExternalMatrixPath = (
                            f"input/sensorAligner/externalMatrices-sensors-aligned.json"
                        )
                        config.moduleAlignAvgMisalignFile = (
                            f"input/moduleAlignment/avgMisalign-aligned.json"
                        )

                    # supply anchor ppoints to all cases
                    config.moduleAlignAnchorPointFile = (
                        f"input/moduleAlignment/anchorPoints.json"
                    )

                    # supply external matrices to all cases
                    config.sensorAlignExternalMatrixPath = (
                        f"input/sensorAligner/externalMatrices-sensors-{fac}.json"
                    )

                    # supply avg misalignments accordingly
                    if misType == "modulesNoRot":
                        config.moduleAlignAvgMisalignFile = (
                            f"input/moduleAlignment/avgMisalign-noRot-{fac}.json"
                        )
                    elif misType == "modulesOnlyRot":
                        config.moduleAlignAvgMisalignFile = (
                            f"input/moduleAlignment/avgMisalign-onlyRot-{fac}.json"
                        )
                    else:
                        config.moduleAlignAvgMisalignFile = (
                            f"input/moduleAlignment/avgMisalign-{fac}.json"
                        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        dest="alignConfig",
        help="find all alignment matrices (IP, corridor, sensors) for runConfig",
    )
    parser.add_argument(
        "-A",
        metavar="--alignConfigPath",
        type=str,
        dest="alignConfigPath",
        help="same as -a, but for all Configs in specified path",
    )

    parser.add_argument(
        "--combi",
        metavar="--combiRunConfig",
        type=str,
        dest="combiRunConfig",
        help="Do a combi run (all misalignments, all alignments in sequence)",
    )
    parser.add_argument(
        "--COMBI",
        metavar="--combiRunConfigPath",
        type=str,
        dest="combiRunConfigPath",
        help="same as --combi, but for all Configs in specified path",
    )

    parser.add_argument(
        "--half",
        metavar="--halfRunConfig",
        type=str,
        dest="halfRunConfig",
        help="Do a half run (simulate/reconstruct data and determine Luminosity)",
    )
    parser.add_argument(
        "--HALF",
        metavar="--halfRunConfigPath",
        type=str,
        dest="halfRunConfigPath",
        help="same as --half, but for all Configs in specified path",
    )

    parser.add_argument(
        "-f",
        metavar="--fullRunConfig",
        type=str,
        dest="fullRunConfig",
        help="Do a full run (simulate mc data, find alignment, determine Luminosity)",
    )
    parser.add_argument(
        "-F",
        metavar="--fullRunConfigPath",
        type=str,
        dest="fullRunConfigPath",
        help="same as -f, but for all Configs in specified path",
    )

    parser.add_argument(
        "-FS",  # TODO: deprecate, wasn't really used
    ...
    parser.add_argument(
        "-l",
        metavar="--lumifitConfig",
        type=str,
        dest="lumifitConfig",
        help="determine Luminosity for runConfig",
    )
    parser.add_argument(
        "-L",
        metavar="--lumifitConfigPath",
        type=str,
        dest="lumifitConfigPath",
        help="same as -l, but for all Configs in specified path",
    )

    parser.add_argument(
        "-el",
        metavar="--extractLumiConfig",
        type=str,
        dest="extractLumiConfig",
        help="extract Luminosity for runConfig",
    )
    parser.add_argument(
        "-EL",
        metavar="--extractLumiConfigPath",
        type=str,
        dest="extractLumiConfigPath",
        help="same as -el, but for all Configs in specified path",
    )

    parser.add_argument(
        "-s",
        metavar="--simulationConfig",
        type=str,
        dest="simulationConfig",
        help="run simulation and reconstruction for runConfig",
    )
    parser.add_argument(
        "-S",
        metavar="--simulationConfigPath",
        type=str,
        dest="simulationConfigPath",
        help="same as -s, but for all Configs in specified path",
    )
    ...
    parser.add_argument(
        "-V",
        metavar="--fitValuesConfigPath",
        type=str,
        dest="fitValuesConfigPath",
        help="display reco_ip and lumi_vals for all runConfigs in path",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        dest="recursive",
        help="use with any config Path option to scan paths recursively",
    )

    parser.add_argument(
        "--hist",
        type=str,
        dest="histSensorAligner",
        help="hist matrix deviations for runConfig",
    )
    parser.add_argument(
        "--histPath",
        type=str,
        dest="histSensorAlignerPath",
        help="hist matrix deviations for all runConfigs in Path, recursively",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        dest="histRecPath",
        help="hist theta rec for well resonctructed tracks",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        dest="makeDefault",
        help="make a single default LMDRunConfig and save it to runConfigs/identity-1.00.json",
    )
    parser.add_argument(
        "-D",
        action="store_true",
        dest="makeMultipleDefaults",
        help="make multiple example LMDRunConfigs",
    )
    parser.add_argument(
        "--debug",
        action="store_true",
        dest="debug",
        help="run single threaded, more verbose output, submit jobs to devel queue",
    )
    parser.add_argument(
        "--updateRunConfigs",
        dest="updateRunConfigs",
        help="read all configs in ./runConfig, recreate the matrix file paths and store them.",
    )
    parser.add_argument(
        "--test", action="store_true", dest="test", help="internal test function"
    )
    ...
    if args.test:
        ...
        if True:
            ...
            alignerMod.readAverageMisalignments(
                "input/moduleAlignment/avgMisalign-1.00.json"
            )
            ...
            comp = moduleComparator(
                LMDRunConfig.fromJSON("runConfigs/special/combi0/15.0/factor-1.00.json")
            )
            comp.loadIdealDetectorMatrices("input/detectorMatricesIdeal.json")
            comp.loadDesignMisalignments(
                "/media/DataEnc2TBRaid1/Arbeit/Root/PandaRoot-New/macro/detectors/lmd/geo/misMatrices/misMat-modules-1.00.json"
            )
            ...
            comp.saveHistogram(
                "input/modulesAlTest/combi0/residuals-modules-2020-03-23-withCombi0-fromHimster.pdf"
            )
