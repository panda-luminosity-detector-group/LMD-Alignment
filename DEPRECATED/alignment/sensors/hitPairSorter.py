...
class hitPairSorter:
    ...
    def sortAll(self):
        ...
        if allThere:
            print(
                f"All npy files already present in {self.targetDir}, skipping sorter!"
            )
            return
        ...
        try:
            for (_, _, arrays) in uproot.iterate(
                lumiPairs,
                "pndsim",
                [
                    b"PndLmdHitPair._overlapID",
                    b"PndLmdHitPair._hit1",
                    b"PndLmdHitPair._hit2",
                ],
