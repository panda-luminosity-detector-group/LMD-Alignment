...
class LMDRunConfig:
    ...
    def generateJobBaseDir(self):
        self.__JobBaseDir = str(
            self.__resolveActual__(
                Path(self.__simDataPath)
                / self.__pathMom__()
                / self.__pathDPM__()
                / self.__pathMisalignDir__()
                / self.__pathTracksNum__()
            )
        )
    ...
    def __resolveActual__(self, globbedPath):
        ...
        if len(result) > 0:
            ...
            if self.useDebug:
                print(
                    f"DEBUG: can't find resolve path on file system, returning globbed path:\n{globbedPath}\n"
                )
            return globbedPath
    ...
    def pathRecoIP(self):
        ...
        return self.__resolveActual__(
            self.__jobBaseDir__()
            / self.__uncut__()
            / self.__alignCorrectionSubDir__()
            / self.__bunches__()
            / self.__recoIP__()
        )
    ...
    def pathLumiVals(self):
        ...
        If you want the extracted lumi for multiSeed (yes, that is possible), always use the UNcorrected branch (set the if to always fail with an 'and False')
        """
        self.__checkMinimum__()
        if (
            self.alignmentCorrection or self.misalignType == "aligned"
        ) or True:  # and False:
            return self.__resolveActual__(
                self.__jobBaseDir__()
                / self.__cut__()
    ...
    def pathDataBaseDir(self):
        self.__checkMinimum__()
        return self.__resolveActual__(
            self.__jobBaseDir__() / self.__cut__() / self.__alignCorrectionSubDir__()
        )
    ...
    def pathTrksQA(self):
        self.__checkMinimum__()
        return self.__resolveActual__(
            self.__jobBaseDir__() / self.__uncut__() / self.__alignCorrectionSubDir__()
        )
