...
class simWrapper:
    ...
    def __init__(self):
        ...
        try:
            ...
            self.logger.log(
                "can't find LuminosityFit installation, PandaRoot installation or Data_Dir!\n"
            )
    ...
    def runSimulations(self):
        ...
        returnVal = subprocess.check_output(
            subProcessCommandTuple, cwd=scriptsPath, env=my_env
    ...
    def GetPendingAndRunning(self, jobID):
        ...
        # find waiting jobs
        squeueOutput = subprocess.check_output(
            ("squeue", "-u", user, "--state=PD", "-r", "-h", "-O", "arrayjobid")
        ).decode(sys.stdout.encoding)
        ...
        # find running jobs
        squeueOutput = subprocess.check_output(
            ("squeue", "-u", user, "--state=R", "-h", "-O", "arrayjobid")
        ).decode(sys.stdout.encoding)
        outputLines = squeueOutput.splitlines()
        for line in outputLines:
            match = re.search(r"^(\d+)", line)
        ...
        self.logger.log(
            f"Thread {self.threadNumber}, JobID: {jobID}: {foundJobsPD} jobs pending, {foundJobsR} running...\n"
        )
    ...
    def waitForJobCompletion(self):
        ...
        if self.currentJobID is None:
            self.logger.log(
                f"can't wait for jobs, this simWrapper doesn't know that jobs to wait for!\n"
            )
            return
        ...
        while True:
            ...
            if foundJobsPD == 0 and foundJobsR == 0:
                print(
                    f"Thread {self.threadNumber}: No more jobs running, continuing..."
                )
                self.logger.log(
                    f"all jobs completed after {waitIntervals * waitIntervalTime / 3600} hours!"
                )
                self.currentJobID = None
                return

            print(
                f"Thread {self.threadNumber}: {foundJobsPD} jobs pending, {foundJobsR} running for IDs: {self.currentJobID}..."
            )
        ...
        self.logger.log(
            f"========= Simulation and Reconstruction done, all Jobs finished."
        )
    ...
    def detLumi(self):
        ...
        self.logger.log(
            f"DEBUG: determining Luminosity of the data set found here:\n{absPath}"
        )
        ...
        returnVal = subprocess.check_output(
            (command, argP, argPval), cwd=scriptsPath
        ).decode(sys.stdout.encoding)
        self.getJobIDfromSubmitOutput(returnVal)
    ...
    def extractLumi(self):
        ...
        if dataPath:
            ...
            if Path(self.config.pathLumiVals()).exists():
                ...
                self.logger.log(
                    f"========= Luminosity extracted!\nLumi values:\n\n{lumiJson}\n\n"
                )
            else:
                self.logger.log(
                    f"========= Luminosity extracted, but lumi_vals.json could not be found!\n"
                )
