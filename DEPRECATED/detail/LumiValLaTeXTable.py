...
class LumiValLaTeXTable(LumiValDisplay):
    def show(self):
        ...
        print(
            "Beam Momentum [GeV/c] & Misalign Type & Misalign Factor & Corrected & Lumi Deviation [\%] \\\\ \\hline"
        )
        for conf in self.configs:
            ...
            if Path(conf.pathLumiVals()).exists():
                ...
                try:
                    lumi = np.round(
                        float(lumiVals["relative_deviation_in_percent"][0]), 3
                    )
                except:
                    lumi = "malformed data!"
            else:
            ...
            print(
                f"{conf.momentum} & {conf.misalignType} & {conf.misalignFactor} & {conf.alignmentCorrection} & {lumi} \\\\"
            )
...
class LumiValGraph(LumiValDisplay):
    ...
    def getAllValues(self, copy=True, reallyAll=False):
        ...
        for conf in self.configs:
            ...
            if (conf.alignmentCorrection != self.corrected) and (
                conf.misalignType != "aligned"
            ):
                raise Exception(
                    f"Error! You cannot mix corrected and uncorrected results!"
                )
            if (conf.misalignType != self.misalignType) and (
                conf.misalignType != "aligned"
            ):
            ...
            if reallyAll and conf.seedID is not None:
                ...
                conf.alMatFile = (
                    f"alMat-combiSenMod-seed{conf.seedID}-{conf.misalignFactor}.json"
                )

                conf.tempDestPath = Path(
                    f"output/temp/LumiVals/multi/{conf.misalignType}-{conf.momentum}-{conf.misalignFactor}-seed{conf.seedID}-{conf.alignmentCorrection}"
                )
            else:
                conf.tempDestPath = Path(
                    f"output/temp/LumiVals/{conf.misalignType}-{conf.momentum}-{conf.misalignFactor}-{conf.alignmentCorrection}"
                )
            ...
            if copy:
                ...
                success = subprocess.run(
                    ["cp", resolved, conf.tempDestPath]
                ).returncode  #! use cp instead of scp for copies from local dirs
            ...
            if reallyAll:
                values.append(
                    [conf.momentum, conf.misalignFactor, lumi, lumiErr, seedID]
                )
            else:
    ...
    def save(self, outFileName, corrected=False):
        ...
        titlesCorr = [
            "Luminosity Fit Error, With Alignment",
            "Luminosity Fit Error, With Alignment",
            "Luminosity Fit Error",
        ]
        titlesUncorr = [
            "Luminosity Fit Error, Without Alignment",
            "Luminosity Fit Error, Without Alignment",
            "Luminosity Fit Error",
        ]
    ...
    def saveAllMomenta(self, outFileName):
        ...
        for i in range(len(sizes)):
            ...
            plt.grid(
                color="grey", which="major", axis="y", linestyle=":", linewidth=0.5
            )
            plt.grid(
                color="grey", which="major", axis="x", linestyle=":", linewidth=0.5
            )
            # plt.legend()
    ...
    def multiSeed(self, fileName):
        ...
        for i in range(len(sizes)):
            ...
            for mom in momenta:
                ...
                ax.errorbar(
                    newArray[:, 1] + offsets[colorI] * offsetscale,
                    newArray[:, 2],
                    yerr=newArray[:, 3],
                    fmt="d",
